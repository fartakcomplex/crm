...
import QuickBackupStatus from '@/components/cms/QuickBackupStatus'
import QuickActionToolbar from '@/components/cms/QuickActionToolbar'
...
const pageComponents: Record<string, React.ComponentType> = {
  ...
  'advanced-settings': dynamic(() => import('@/components/cms/AdvancedSettingsPage'), { loading: LoadingFallback, ssr: false }),
...
function SidebarNav({
  ...
  return (
    <>
      ...
      <ScrollArea className="flex-1 min-h-0 py-2 scrollbar-sidebar">
...
function AppContent() {
  ...
  useEffect(() => {
    const onboardingDone = localStorage.getItem('cms_onboarding_done')
    if (!onboardingDone) {
      // eslint-disable-next-line react-hooks/set-state-in-effect -- intentional: check localStorage on mount
  ...
  return (
    <TooltipProvider delayDuration={0}>
      ...
      <ActivityFeedWidget maxItems={8} refreshInterval={30000} />
