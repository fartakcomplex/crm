...
export default function AIContentStudio() {
  ...
  // ─── Research: Web search before media generation (non-blocking with timeout) ──
  const researchContent = useCallback(async (title: string, content?: string, type: 'image' | 'video' | 'audio' = 'image'): Promise<string> => {
    if (!title.trim()) return ''

    setIsResearching(true)
    try {
      // Race research against a 5s timeout — if it takes longer, skip it
      ...
      const res = await fetch('/api/ai/research', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ title, content: content?.substring(0, 200), type }),
      ...
      if (!res.ok) {
        // Rate limited or error — skip research silently
        console.log('[research] API returned', res.status)
        return ''
      }
      ...
      if (data.researchContext) {
        if (data.fallback) {
          setResearchInfo('بدون تحقیق (محدودیت سرور)')
        } else {
          setResearchInfo(`${data.searchResults?.length || 0} منبع بررسی شد ✅`)
        }
      ...
      // Timeout or network error — skip research silently
      if (err instanceof DOMException && err.name === 'AbortError') {
        console.log('[research] Timed out, skipping')
      } else {
  ...
  const generateVideo = useCallback(async (feature: AIFeature, data: Record<string, string>) => {
    ...
    // Step 2: Poll GET every 10 seconds (backend does 10s polling, client polls slightly slower)
    // Max attempts: 200 × 10s = 33 minutes (generous for 10s videos with backoff)
    ...
    for (let i = 0; i < maxAttempts; i++) {
      ...
      try {
        ...
        if (!pollRes.ok) {
          ...
          // 400 = backend error (generation failed)
          if (pollRes.status === 400) throw new Error(err.userMessage || err.error || 'تولید ویدئو ناموفق بود')
          continue
        }

        const result = await pollRes.json()

        // SUCCESS with valid videoUrl
        ...
        // SUCCESS without videoUrl — this shouldn't happen but handle gracefully
        if (result.status === 'success' && !result.videoUrl) {
          console.warn('[video-gen] Backend reported success but no videoUrl. Debug info:', result.debugInfo || result.metadata)
          throw new Error('ویدئو تولید شد اما آدرس آن دریافت نشد. لطفاً دوباره تلاش کنید.')
        }
        ...
        // Only re-throw fatal errors, continue on network errors
        if (pollErr instanceof Error) {
          const msg = pollErr.message
          if (msg.includes('زمان') || msg.includes('یافت نشد') || msg.includes('ناموفق') ||
              msg.includes('فیلتر') || msg.includes(' ایمنی') || msg.includes('بارگذاری بالاست') ||
              msg.includes('دریافت نشد')) {
            throw pollErr
          }
        }
